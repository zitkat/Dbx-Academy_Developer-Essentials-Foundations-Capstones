...
def getTag(tagName: str, defaultValue: str = None) -> str:
  tags = sc._jvm.scala.collection.JavaConversions.mapAsJavaMap(
...
def validate_registration_id(registration_id):
  try: 
    return BI.len(registration_id) > 10
  except: 
    raise Exception(f"The specified Registration ID ({registration_id}) is invalid.<br/>Reinstall the source datasets to address this problem.")
...
class CapstoneLogger:
  ...
  def __init__(self):
    self.capstone_name = "developer-foundations-capstone-v3"
  ...
  def logCompletion(self, registration_id:str, email_address:str, exercise:int):
    ...
    assert response.status_code == 200, f"Expected HTTP response code 200, received {response.status_code}\n{response.text}"
...
try:
  registration_id = spark.read.json(f"{working_dir}/raw/_meta/config.json").first()["registration_id"]
except:
